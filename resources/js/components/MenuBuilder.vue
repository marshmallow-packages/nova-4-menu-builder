<template>
  <vue-nestable :value="value" :max-depth="maxDepth" @input="value = $event" @change="$emit('onMenuChange')" :hooks="{
    beforeMove: beforeMove,
  }" class="px-3" classProp="classProp">
    <template v-slot="{ item }">
      <vue-nestable-handle
        ...
        <div class="buttons o1-gap-x-2 md:o1-w-1/3 o1-flex o1-justify-end o1-content-center">
          <button :title="__('novaMenuBuilder.edit')" @click.prevent="$emit('editMenu', item)"
            class="text-gray-500 dark:text-gray-400 hover:[&:not(:disabled)]:text-primary-500 dark:hover:[&:not(:disabled)]:text-primary-500">
            <Icon type="pencil-alt" />
          </button>

          <button :title="__('novaMenuBuilder.duplicate')" @click.prevent="$emit('duplicateMenuItem', item)"
            class="text-gray-500 dark:text-gray-400 hover:[&:not(:disabled)]:text-primary-500 dark:hover:[&:not(:disabled)]:text-primary-500">
            <Icon type="duplicate" />
          </button>

          <button :title="__('novaMenuBuilder.delete')" @click.prevent="$emit('removeMenu', item)"
            class="text-gray-500 dark:text-gray-400 hover:[&:not(:disabled)]:text-primary-500 dark:hover:[&:not(:disabled)]:text-primary-500">
            <Icon type="trash" />
          </button>
        </div>
